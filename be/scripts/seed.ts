...
async function seed() {
  ...
  for (const b of books) {
    const existing = await client.query('SELECT id FROM books WHERE title = $1 AND "isDeleted" = false', [b.title])
    if (existing.rows.length === 0) {
      await client.query(
        'INSERT INTO books (title, author, category, "purchasePrice", "sellingPrice", stock, "minStock", "isDeleted", created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, false, NOW(), NOW())',
        [b.title, b.author, b.category, b.purchasePrice, b.sellingPrice, b.stock, b.minStock]
      )
